...
from main import main, parse_file, process_clickbait
...
def test_no_duplicate_records_when_same_file_passed_twice(capsys, monkeypatch):
    ...
    main()
    out = capsys.readouterr().out
    assert out.count("Секрет который скрывают тимлиды") == 1, (
        "Найдено дублирующееся видео."
    )
...
def test_main_clickbait_full_flow(capsys, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["main.py", "--files", "stats1.csv", "stats2.csv", "--report", "clickbait"],
    )
