...
Утилита для обработки .csv файлов, содержащих информацию о видео
с полями: title,ctr,retention_rate,views,likes,avg_watch_time.
Выводит отчеты по данным файлам.
На данный момент реализован только отчет clickbait, который позволяет
увидеть список видео, отсортированный по убыванию ctr с характеристиками
ctr > 15 и retention_rate < 40""")
    ...
    parser.add_argument(
        "--files",
        nargs="*",
        help="Названия файлов для обработки, например: 'stats1.csv stats2.csv', '*.csv'",
    )
    parser.add_argument(
        "--report", nargs="*", help="Название выводимого отчета, например: 'clickbait'"
    )
